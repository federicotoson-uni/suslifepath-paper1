...
H = np.logspace(np.log10(25), np.log10(1000), 400)
...
for s, (e, t, r) in res.items():
    ...
    if len(k):
        i = k[0]
        hstar = H[i-1] + (1-r[i-1])*(H[i]-H[i-1])/(r[i]-r[i-1]) if i else H[0]
    else: hstar = float("nan")
...
fig, (ax_a, ax_b) = plt.subplots(2, 1, figsize=(3.35, 3.0), constrained_layout=True, sharex=True)
for ax in (ax_a, ax_b):
...
k = np.where(r10 >= 1)[0]; i = k[0]
hstar = H[i-1] + (1-r10[i-1])*(H[i]-H[i-1])/(r10[i]-r10[i-1]) if i else H[0]
